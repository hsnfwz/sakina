function Button({ children, handleClick, isDisabled, isLoading, color }) {
  return (
    <button
      ...
      className={`${color ? `${color.tailwindBackgroundColor} ${color.tailwindHoverBackgroundColor} ${color.tailwindTextColor} ${color.tailwindFillColor} ${color.tailwindBorderColor} ${color.tailwindHoverBorderColor}` : 'border-transparent bg-transparent fill-neutral-500 text-neutral-500'} flex cursor-pointer items-center justify-center gap-2 self-start rounded-full border-2 p-2 focus:z-50 focus:ring-0 focus:outline-2 focus:outline-black disabled:pointer-events-none disabled:opacity-50`}
    >
      {isLoading && (
