function Button({ children, handleClick, isDisabled, isLoading, buttonColor }) {
  return (
    <button
      ...
      disabled={isDisabled}
      onClick={handleClick}
      className={`${buttonColor ? `${buttonColor.tailwindColor} ${buttonColor.tailwindColorHover} text-white` : 'bg-transparent text-black hover:bg-neutral-200'} flex w-full items-center justify-center gap-2 self-start rounded-lg border-2 border-transparent fill-white p-2 focus:border-2 focus:border-black focus:outline-none focus:ring-0 disabled:pointer-events-none disabled:opacity-50`}
    >
      {isLoading && (
